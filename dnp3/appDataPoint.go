...
func (p *Point1Bit) FromBytes(data []byte, prefSize int) error {
	if len(data) > 1 {
		...
		return fmt.Errorf("can't have a prefix on 1 bit packed points")
	}
...
func newPoints1Bit(d []byte, num, prefSize int) ([]Point, int, error) {
	if num > (8 * len(d)) {
		...
		return nil, 0, fmt.Errorf("prefix size must be 0 for packed bits")
	}
...
func (p *Point1BitFlags) FromBytes(data []byte, prefSize int) error {
	if len(data) > 1+prefSize {
		return fmt.Errorf("can't construct 1 bit point with flags from multiple bytes")
	}
	if prefSize > 0 {
...
func newPoints1BitFlags(d []byte, num, prefSize int) ([]Point, int, error) {
	...
	for i := range num {
		p := &Point1BitFlags{}
		if err := p.FromBytes([]byte{d[i]}, prefSize); err != nil {
			return o, num, fmt.Errorf("could not decode point: 0x % X, err: %v",
				d[i], err)
		}
		o = append(o, p)
	}
...
func (p *Point2Bits) FromBytes(data []byte, prefSize int) error {
	if len(data) > 1 {
		...
		return fmt.Errorf("can't have prefix on 2 bit packed points")
	}
...
func newPoints2Bits(d []byte, num, prefSize int) ([]Point, int, error) {
	if num > (8 * len(d) / 2) {
		...
		return nil, 0, fmt.Errorf("can't have a prefix for 2 bit packed")
	}
...
func (p *PointNBytesFlags) FromBytes(d []byte, prefSize int) error {
	...
	p.Flags.FromByte(d[prefSize])
	p.Value = d[prefSize+1:]
	return nil
}

func (p *PointNBytesFlags) ToBytes() []byte {
	o := append(p.Prefix, p.Flags.ToByte())
...
func newPointsGeneric(newPoint func() Point, data []byte, width, num, prefSize int) ([]Point, int, error) {
	size := num * (prefSize + width)
	if size > len(data) {
		return nil, 0, fmt.Errorf("not enough bytes for %d %d-byte points with %d-byte prefix", num, width, prefSize)
	}
	var o []Point
	for i := range num {
		p := newPoint()
...
func constructorNoPoints(d []byte, num, prefSize int) ([]Point, int, error) {
	if num != 0 {
...
func (f *PointFlags) FromByte(data byte) error {
	...
	if f.Reserved {
		return fmt.Errorf("reserved bit must be 0")
	} else {
		return nil
	}
}
